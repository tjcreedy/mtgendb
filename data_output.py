...
parser.add_argument('-q', '--query', dest='mysql_query', metavar='custom_MySQL_query',
                    help="Custom MySQL query to extract data from database. (e.g. \"SELECT * FROM metadata WHERE country='United Kingdom';\") NOTE: Your custom specification must be enclosed by double quotations, as above.")
...
parser_count.add_argument('-s', '--specifications', dest='mysql_specs', metavar='{specifications}', nargs='+',
                          help="Comma-separated list of mysql specifications.\n(e.g. 'subregion=Sabah' 'length>25000' 'order=Coleoptera') REMEMBER: Each individual spec must be enclosed by quotations and separated from the next by a space as above.")

# Create the parser for the 'CSV' command
parser_csv = subparsers.add_parser('CSV', help='Ouputs a .csv file.')
parser_csv.add_argument('-o', '--out', dest='output_name', metavar='{Output filename}', required=True,
                        help="Preferred filename for the output (extension will be added automatically according to your output format choice).")
parser_csv.add_argument('-t', '--table', dest='database_table', metavar='{Table name}',
                        choices=["metadata", "bioentry", "bioentry_dbxref", "bioentry_qualifier_value",
                                 "bioentry_reference", "biosequence", "seqfeature", "comment", "taxon", "taxon_name"],
                        help="Name of database table you wish to extract data from. (e.g. metadata.) If provided, the script will assume you require data from every column in the specified table. If this is not the case then stating only the required columns under the -c flag will suffice.")
parser_csv.add_argument('-c', '--columns', dest='table_columns', metavar='{Column name(s)}', nargs='+', default=['*'],
                        help="Name of table columns you wish to extract data from. (e.g. name length description)")
parser_csv.add_argument('-s', '--specifications', dest='mysql_specs', metavar='{Specification(s)}', nargs='+',
...
parser_fasta.add_argument('-o', '--out', dest='output_name', metavar='{Output filename}', required=True,
                          help="Preferred filename for the output (extension will be added automatically according to your output format choice).")
parser_fasta.add_argument('-s', '--specifications', dest='mysql_specs', metavar='{Specification(s)}', nargs='+',
                          help="Comma-separated list of mysql specifications.\n(e.g. 'subregion=Sabah' 'length>25000' 'order=Coleoptera') NOTE: Each individual specification must be enclosed by quotations and separated from the next by a space as above.")
parser_fasta.add_argument('-g', '--gene', dest='gene', metavar='{gene_name}',
                          help='Name of CDS/gene you wish to extract.')

# Create the parser for the 'GB' command
parser_gb = subparsers.add_parser('GB', help="Outputs an annotated .gb file.")
parser_gb.add_argument('-o', '--out', dest='output_name', metavar='{Output filename}', required=True,
                       help="Preferred filename for the output (extension will be added automatically according to your output format choice).")
parser_gb.add_argument('-s', '--specifications', dest='mysql_specs', metavar='{Specification(s)}', nargs='+',
                       help="Comma-separated list of mysql specifications.\n(e.g. 'subregion=Sabah' 'length>25000' 'order=Coleoptera') NOTE: Each individual specification must be enclosed by quotations and separated from the next by a space as above.")
